...
export function PlanOverview({ plan }: { plan: PlanWithWeeks }) {
  ...
  return <section className="space-y-5">
    <div className="hero"><div><p className="eyebrow">Active therapy plan</p><h1>{plan.title}</h1><p>{plan.summary}</p><div className="actions"><ExportPlanPdfButton /><Link href={`/plan/${plan.id}/print`} className="linkbutton secondary no-print">Printable plan view</Link></div></div><div className="progress"><strong>{done}/{sessions.length}</strong><span>sessions complete</span></div></div>
