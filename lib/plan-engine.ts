...
import { daysForFrequency, effortAdjustmentDirection, nextTargetReps } from "./therapy-rules";
...
export async function adjustFutureTargets(sessionId: string, effort: number) {
  ...
  const direction = effortAdjustmentDirection(effort, (logs || []).map((log) => log.perceived_effort));
  ...
  for (const target of targets || []) {
    ...
    const after = nextTargetReps(before, direction);
    await supabase.from("session_exercises").update({ reps: after }).eq("id", target.id);
    await supabase.from("audit_logs").insert({ actor_type: "rule-engine", action: direction === "reduce" ? "reduce_reps_10_percent" : "increase_reps_10_percent", object_type: "session_exercise", object_id: target.id, before_json: { reps: before }, after_json: { reps: after } });
  }
}
