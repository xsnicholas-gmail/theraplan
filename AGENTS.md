...
TheraPlan is a physiotherapy and strength/mobility training app that generates personalised weekly exercise plans from user health intake, tracks workout completion with timers and rep logging, and adapts the plan based on feedback.
...
- **Commit as your GitHub identity, or Vercel will block the deploy.** Vercel verifies that
  ...
  ```
  git config user.email "281070086+xsnicholas-gmail@users.noreply.github.com"
  git config user.name "xsnicholas-gmail"
  ```
